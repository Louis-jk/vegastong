...
const Detail = (props) => {
    ...
    VegasGet(`/api/talk/get_talk/${tk_id}`)
      .then((res) => {
        ...
        if (res.result === 'success') {
          setContentWordCount(res.data.wo_count)
          setNickName(res.data.ut_nickname)
          setDivision(res.data.tk_division)
          setContentArId(res.data.ar_id)
          setTagTravel(res.data.ar_tag_travel)
          setTagRestaurant(res.data.ar_tag_restaurant)
          setTagCafe(res.data.ar_tag_cafe)
          setTagShop(res.data.ar_tag_shop)
          setTkContent(res.data.tk_content)
  ...
  const getReplyAPI = () => {
    // axios({
    //   method: 'get',
    //   url: `${BASE_URL}/api/word/get_words`,
    //   data: qs.stringify({
    ...
    //   },
    // })
    VegasGet(`/api/word/get_words?wo_category=talk&wo_ref_id=${tk_id}&page=1`)
      .then((res) => {
        if (res.result === 'success') {
  ...
  const delAPI = async () => {
    await VegasGet(`/api/talk/remove_talk/${tk_id}`, {
        ...
        <Content>
          ...
          {files.length > 1 ? (
            ...
            <View style={{ marginHorizontal: 20 }}>
              <Image
                source={{
                  uri: `${BASE_URL}/${files[0].ft_file_path}`
                }}
                resizeMode='cover'
                style={{
                  width: width - 40,
