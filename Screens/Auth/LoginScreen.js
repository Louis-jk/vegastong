...
const SOCIAL_CHECK = baseUrl + '/api/user/social_login'
...
const LoginScreen = (props) => {
  ...
  const socialCheckApi = async (type, id) => {
    ...
    try {
      const check = await axios.post(SOCIAL_CHECK, form, {
        headers: {
          'api-secret':
  ...
  const socialLoginPass = async (data) => {
    ...
    const token_update = await axios.post(
      `${baseUrl}/api/user/update_fcm_token`,
      form,
      { headers }
    )

    if (token_update.data.result === 'success') {
      _storeToken(token)
  ...
  const sendFcmToken = (payload) => {
    ...
    axios({
      method: 'post',
      url: `${baseUrl}/api/user/update_fcm_token`,
      headers: {
      ...
      .then((res) => {
        ...
        if (res.data.result === 'fail') {
          Alert.alert(`${res.data.error}`, '관리자에게 문의하세요.', [
            {
              text: '확인',
              onPress: () => {}
            }
    ...
    if (userPw.length < 4) {
      ...
      axios({
        ...
        .then((res) => {
          ...
          if (res.data.result === 'fail') {
            Alert.alert(`${res.data.error}`, '재가입하시겠습니까?', [
              {
                text: '재가입하기',
                onPress: () =>
          ...
          if (res.data.result === 'fail') {
            if (res.data.error === '가입되지 않은 아이디입니다.') {
              setError(res.data.error)
            } else if (res.data.error === '비밀번호를 잘못입력하셨습니다.') {
              setError(res.data.error)
            } else {
              return false
            }
          } else {
            setExeption(res.data.error)
          }
        })
